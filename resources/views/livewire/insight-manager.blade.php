<div x-data="{ loaded: true }">
                    ...
                    x-data="{ 
                        ...
                        handleMouseUp(e) {
                            ...
                            if (deltaX < 5 && deltaY < 5) {
                                // Verifica se não há texto selecionado
                                const selection = window.getSelection();
                                if (!selection || selection.toString().length === 0) {
                                    $wire.openModal({{ $insight['id'] }});
                                }
                            }
                ...
                >
                    <div class="flex justify-between items-start mb-3 flex-shrink-0">
                        ...
                        <div class="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                            <button
                                wire:click.stop="openModal({{ $insight['id'] }})"
                                @mousedown.stop
                                @mouseup.stop
                            ...
                            <button
                                wire:click.stop="confirmDelete({{ $insight['id'] }})"
                                @mousedown.stop
                                @mouseup.stop
                    ...
                    <div class="text-sm text-gray-400 flex items-center flex-shrink-0">
                        ...
                        {{ \Carbon\Carbon::parse($insight['created_at'])->diffForHumans() }}
                    </div>
                </div>
            @endforeach
        </div>
    ...
    @if($toast['show'])
        <div
            ...
            x-show="show"
            x-transition
            class="fixed top-20 right-4 bg-white rounded-lg shadow-lg p-4 z-50 {{ $toast['type'] === 'success' ? 'border-l-4 border-green-500' : 'border-l-4 border-red-500' }}"
        >
            <p class="font-medium {{ $toast['type'] === 'success' ? 'text-green-800' : 'text-red-800' }}">
                {{ $toast['message'] }}
            </p>
        </div>
    @endif
            ...
            <div
                ...
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-2xl font-bold text-gray-800">
                        {{ $editingId ? 'Editar Insight' : 'Novo Insight' }}
                    </h2>
                    <button
    ...
    @if($showConfirmDelete)
        <div
            ...
            @keydown.escape.window="show = false; setTimeout(() => $wire.$set('showConfirmDelete', false), 200)"
            x-show="show"
            x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100"
